...
@implementation TCourseHeaderView
...
//        make.height.equalTo(@29);
    ...
    if([self.delegate respondsToSelector:@selector(headerViewHeightChange:)]){
        NSLog(@"%f",self.tagView1.intrinsicContentSize.height);
        [self.delegate headerViewHeightChange:self.tagView1.intrinsicContentSize.height];
    }
...
- (SKTagView*)tagView1{
    if (_tagView1==nil) {
        _tagView1 = ({
            ...
            view.padding = UIEdgeInsetsMake(15, 10, 7.5, 10);
            view.interitemSpacing = 7.5;
            view.lineSpacing = 7.5;
            view.preferredMaxLayoutWidth = [UIScreen mainScreen].bounds.size.width-65;
